// EventScreen base + all subclasses
// Ported from EventScreen.pde
...
class EventScreen extends Screen {
  ...
  render() {
    ...
    textSize(18);
    textAlign(LEFT, TOP);
    text(this.getDisplayText(), 10, 10, this.BOX_WIDTH - 20, this.BOX_HEIGHT - 20);
...
class TeleportScreen extends EventScreen {
  constructor(teleportText, destination) {
    super();
...
class MoveToScreen extends EventScreen {
  constructor(moveText, destination) {
    super();
    this._text = moveText;
    this._destination = destination;
  }
...
class SectorArrivalScreen extends EventScreen {
  constructor(arrivalText, sectorName) {
    super();
